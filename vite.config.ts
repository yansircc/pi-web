...
export default defineConfig(({ mode }) => ({
  ...
  run: {
    tasks: {
      ...
      "ci:verify": {
        command: ["vp check", "vp run ci:typecheck", "vp test", "vp run ci:effect", "pnpm test:e2e"],
        cache: false,
      },
    },
  },
  ...
  define: {
    ...
    __PI_VERSION__: JSON.stringify(pkg.dependencies["@earendil-works/pi-coding-agent"]),
  },
  server: {
    port: 30141,
  },
