...
export const ChromeStatusProjection = Schema.Struct({
  ...
  authorization: Schema.Union([Schema.Literals(["indefinite", "locked"]), Schema.Struct({ expiresAt: Schema.Number })]),
...
const WeixinBindings = Schema.Array(WeixinBindingStatus).check(
  Schema.makeFilter((bindings) => new Set(bindings.map((binding) => binding.sessionId)).size === bindings.length, {
    message: "Each session can have at most one Weixin binding",
  }),
  Schema.makeFilter(
    (bindings) => {
      const accountIds = bindings.flatMap((binding) => (binding.accountId === undefined ? [] : [binding.accountId]))
